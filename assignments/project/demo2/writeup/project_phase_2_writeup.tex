...
\begin{quote}
\texttt{./run test [-v] project phase\_2\_x [-only] [-direct|-associative]}
\end{quote}
\texttt{x} is the follow-on phase to be tested.
\texttt{-only} runs only that follow-on category plus \texttt{student\_custom}.
\texttt{-direct} and \texttt{-associative} are valid for \texttt{phase\_2\_3} and select a single cache implementation to test.
...
\begin{quote}
\texttt{./run test project phase\_2\_1 -only}
\end{quote}
This runs the Phase 2.1 category (which corresponds to the unaligned-access set) plus \texttt{student\_custom}.
...
\begin{center}
\begin{figure}[h]
\includegraphics[width=\textwidth]{memory-info.png}
\end{figure}
\end{center}
...
\begin{quote}
\texttt{./run test project phase\_2\_2 -only}
\end{quote}
...
\begin{figure}
\includegraphics{four_bank_mem.png}
...
\begin{figure}
\includegraphics[width=\textwidth]{cache_org.png}
...
Run only the 2.3 category (plus \texttt{student\_custom}):
\begin{quote}
\texttt{./run test project phase\_2\_3 -only}
\end{quote}

Test only one implementation:
\begin{quote}
\texttt{./run test project phase\_2\_3 -direct}
\end{quote}
\begin{quote}
\texttt{./run test project phase\_2\_3 -associative}
\end{quote}
...
\item If one way is invalid, install in the invalid way.
\end{enumerate}
