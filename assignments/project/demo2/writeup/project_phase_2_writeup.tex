...
\subsection*{Signal Interactions}

When \texttt{enable} is high, the direct-mapped cache behavior is determined by
the \texttt{comp} and \texttt{write} control signals.
...
\hline
\textbf{Mode} & \textbf{When Used} & \textbf{Behavior} \\
\hline
Compare Read \newline (\texttt{comp=1, write=0}) &
Load instructions &
\parbox[t]{\linewidth}{\raggedright
$\bullet$ \texttt{tag\_in}, \texttt{index}, and \texttt{offset} must be valid.\\
$\bullet$ A hit or miss is reported in the same cycle.\\
$\bullet$ On a hit, \texttt{data\_out} returns the selected word.\\
$\bullet$ On a miss, \texttt{valid} and \texttt{dirty} describe the current line.\\[0.15em]} \\
\hline
Compare Write \newline (\texttt{comp=1, write=1}) &
Store instructions &
\parbox[t]{\linewidth}{\raggedright
$\bullet$ \texttt{data\_in}, \texttt{tag\_in}, \texttt{index}, and \texttt{offset} must be valid.\\
$\bullet$ On a hit, the selected word is written at the rising edge.\\
$\bullet$ The dirty bit is set to 1 on a hit.\\
$\bullet$ A hit on an invalid line should still be treated as a miss.\\
$\bullet$ During a compare-write hit, \texttt{dirty} is not meaningful.\\[0.15em]} \\
\hline
Access Read \newline (\texttt{comp=0, write=0}) &
Victim inspection / writeback preparation &
\parbox[t]{\linewidth}{\raggedright
$\bullet$ The cache behaves like a RAM.\\
$\bullet$ \texttt{index} and \texttt{offset} select the line and word to inspect.\\
$\bullet$ \texttt{data\_out}, \texttt{tag\_out}, \texttt{valid}, and \texttt{dirty} are meaningful in the same cycle.\\
$\bullet$ Use this mode when checking a victim line before writeback.\\[0.15em]} \\
\hline
Access Write \newline (\texttt{comp=0, write=1}) &
Cache-line fill from memory &
\parbox[t]{\linewidth}{\raggedright
$\bullet$ \texttt{index}, \texttt{offset}, \texttt{tag\_in}, \texttt{valid\_in}, and \texttt{data\_in} must be valid.\\
$\bullet$ At the rising edge, the cache writes the incoming data and tag.\\
$\bullet$ The valid bit is set.\\
$\bullet$ The dirty bit is cleared to 0.\\[0.15em]} \\
\hline
\end{tabular}
\endgroup
\end{center}
...
When \texttt{enable} is high, the two-way set-associative cache still uses the
\texttt{comp} and \texttt{write} control signals, but now both cache ways must
...
Load instructions &
\parbox[t]{\linewidth}{\raggedright
$\bullet$ Drive \texttt{index} and \texttt{offset} to both cache modules.\\
$\bullet$ A hit occurs if either hit output is high.\\
$\bullet$ Use the hit result to select the correct \texttt{data\_out}.\\
$\bullet$ On a miss, choose a victim way: invalid way first, way 0 if both are invalid, otherwise use the pseudo-random policy.\\[0.15em]} \\
\hline
Compare Write \newline (\texttt{comp=1, write=1}) &
Store instructions &
\parbox[t]{\linewidth}{\raggedright
$\bullet$ Drive \texttt{index}, \texttt{offset}, and \texttt{data\_in} to both cache modules.\\
$\bullet$ A hit occurs if either hit output is high.\\
$\bullet$ Only one cache module should write on a hit.\\
$\bullet$ Your control logic must ensure the same line is never present in both ways.\\[0.15em]} \\
\hline
Access Read \newline (\texttt{comp=0, write=0}) &
Victim inspection / writeback preparation &
\parbox[t]{\linewidth}{\raggedright
$\bullet$ After choosing the victim way, use that select signal to mux the outputs.\\
$\bullet$ Read \texttt{data\_out}, \texttt{valid}, and \texttt{dirty} from the selected way.\\
$\bullet$ Use this mode when checking whether the chosen victim must be written back.\\[0.15em]} \\
\hline
Access Write \newline (\texttt{comp=0, write=1}) &
Cache-line fill from memory &
\parbox[t]{\linewidth}{\raggedright
$\bullet$ Drive \texttt{index}, \texttt{offset}, \texttt{data\_in}, and \texttt{valid\_in} to both cache modules.\\
$\bullet$ Assert write only for the selected way.\\
$\bullet$ Fill the chosen cache way without modifying the other one.\\[0.15em]} \\
\hline
\end{tabular}
\endgroup
\end{center}
...
\begin{figure}[h]
\centering
\includegraphics[width=\textwidth]{src/victimway-eg.png}
\end{figure}
...
\subsection*{Synthesis}
You will need to run synthesis on both your direct-mapped and set-associative caches and verify that neither produces any errors. You should turn in all of the reports generated by synthesis.

Your synthesis results should be placed in these directories:
\begin{quote}
\texttt{./demo2/verilog/phase2\_3/cache\_direct/}
...
For Phase 2.3, your work must be present in:
\begin{quote}
\texttt{./demo2/verilog/phase2\_3/cache\_direct/}
...
Required artifacts for the cache demo include:
...
Use the phase-specific submission wrappers:
\begin{quote}
\texttt{./run build\_submission project phase\_2}

\texttt{./run build\_submission project phase\_2\_1}

\texttt{./run build\_submission project phase\_2\_2}

\texttt{./run build\_submission project phase\_2\_3}
\end{quote}

Each command packages the expected files for that phase and produces a submission zip for upload.

Generated submission zips are written to the corresponding turnin directories:
\begin{quote}
\texttt{./generated\_turnins/project\_phase\_2/}

\texttt{./generated\_turnins/project\_phase\_2\_1/}

\texttt{./generated\_turnins/project\_phase\_2\_2/}

\texttt{./generated\_turnins/project\_phase\_2\_3/}
\end{quote}
